...
{chart_decade_eq}
<h3>新エッジ源の探索結果：H1は否定的、クロス銘柄分散は肯定的</h3>
<p>1時間足（76,528本）でブログ由来の2仮説——「H1 200EMAタッチ反発」と「前日高安ラインへのスプリング」——を事前登録検証しましたが、24構成すべてがテスト窓（2022年以降）でマイナスとなり不採用。平均回帰系はレンジ期に微プラス・トレンド期に全滅という、日足トレンド系の正確な鏡像を示しました（どのスタイルが効くかはレジームが決める）。</p>
<p>一方、凍結ルールを未使用のEURJPY・USDJPYに無変更で転写したところ、<b>3ペアすべてが「上昇レジームで勝つ」同一プロファイルを再現</b>（EURJPY通期+30.7%・USDJPY+9.1%）。勝ち年がペア間でずれるため、追加パラメータゼロの<b>等ウェイト3ペアポートフォリオ</b>では通期シャープ+0.28・+17.2%・<b>最大DD10.2%（単体の半分以下）</b>、2022年以降はシャープ+0.91・+23.9%、トレード数92件/10年（3倍）となりました。詳細：results/h1_research/・results/cross_instrument/。</p>
<h3>参考：改良探索（トレード数増×勝率向上・日足系）の結果は否定的</h3>
